...
function s.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	if Duel.SendtoHand(sg,nil,REASON_EFFECT)==4 then
		...
		Duel.BreakEffect()
		local dg=Duel.SelectMatchingCard(tp,Card.IsAbleToHand,tp,LOCATION_HAND,0,2,2,nil)
		Duel.SendtoDeck(dg,nil,SEQ_DECKSHUFFLE,REASON_EFFECT)
	end
end
function s.filter0(c)
	return c:IsType(TYPE_MONSTER) and c:IsCanBeFusionMaterial() and c:IsAbleToRemove() and c:IsSetCard(0xbf,0xc0)
end
