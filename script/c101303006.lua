...
function s.sptg(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	if chk==0 then
		local chkf=tp
		local mg1=Duel.GetFusionMaterial(tp):Filter(Card.IsRace,nil,RACE_ROCK)
...
function s.tgfilter(c)
	return c:IsSetCard(0x2066) and c:IsLevel(8) and c:IsAbleToGrave()
end
...
function s.atkcon(e,tp,eg,ep,ev,re,r,rp)
	...
	return a:IsFaceup() and a:IsAttribute(ATTRIBUTE_EARTH)
		or b:IsFaceup() and b:IsAttribute(ATTRIBUTE_EARTH)
end
