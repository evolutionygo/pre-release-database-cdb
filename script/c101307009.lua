...
function s.cfilter(c)
	return c:IsFaceup() and c:IsSetCard(0x2f2)
end
function s.spcon(e,c)
	if c==nil then return true end
	return Duel.GetLocationCount(c:GetControler(),LOCATION_MZONE)>0 and
		Duel.IsExistingMatchingCard(s.cfilter,c:GetControler(),LOCATION_ONFIELD,0,1,nil)
end
function s.thfilter(c)
	return (c:IsSetCard(0x2f2) and c:IsType(TYPE_SPELL+TYPE_TRAP) or c:IsSetCard(0x19e)) and c:IsAbleToHand()
end
