...
function s.rmfilter(c,tp)
	...
	if ct>=2 then return c:IsAbleToRemove(tp,POS_FACEDOWN) and g:FilterCount(Card.IsAbleToRemove,nil,tp,POS_FACEDOWN)==ct end
	return false
end
function s.target(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	...
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_REMOVE,tp)
...
function s.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	if tc:IsFaceup() and tc:IsRelateToEffect(e) then
		local g=Duel.GetMatchingGroup(Card.IsCode,tp,0,LOCATION_GRAVE,1,nil,tc:GetCode())
		local ct=g:GetCount()
		if ct==1 then
		...
		elseif ct>=2 then
			if g:FilterCount(Card.IsAbleToRemove,nil,tp,POS_FACEDOWN)~=ct then return end
			g:AddCard(tc)
