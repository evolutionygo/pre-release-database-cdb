...
function s.tgtg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	...
	if chk==0 then return c:IsCanRemoveCounter(tp,0x6f,1,REASON_COST) 
		and Duel.IsExistingTarget(Card.IsFaceup,tp,0,LOCATION_MZONE,1,nil) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_FACEUP)
	local tc=Duel.SelectTarget(tp,Card.IsFaceup,tp,0,LOCATION_MZONE,1,1,nil):GetFirst()
	local b1=c:IsCanRemoveCounter(tp,0x6f,1,REASON_COST) 
	local b2=not tc:IsAttack(0) and c:IsCanRemoveCounter(tp,0x6f,2,REASON_COST) 
	local b3=aux.NegateEffectMonsterFilter(tc) and c:IsCanRemoveCounter(tp,0x6f,3,REASON_COST) 
	local b4=c:IsCanRemoveCounter(tp,0x6f,4,REASON_COST) 
	local op=aux.SelectFromOptions(tp,
			{b1,aux.Stringid(id,1),1},
			{b2,aux.Stringid(id,2),2},
			{b3,aux.Stringid(id,3),3},
			{b4,aux.Stringid(id,4),4})
	c:RemoveCounter(tp,0x6f,op,REASON_COST)
