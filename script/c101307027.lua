...
function s.tgcost(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	if chk==0 then return Duel.IsExistingMatchingCard(s.costfilter,tp,LOCATION_HAND,0,1,c) and c:IsAbleToGraveAsCost() end
...
function s.thfilter(c)
	return not c:IsAttribute(ATTRIBUTE_FIRE) and c:IsSetCard(0x2f1)
...
function s.cfilter(c)
	return c:IsAttribute(ATTRIBUTE_WATER) and c:IsSetCard(0x2f1)
end
